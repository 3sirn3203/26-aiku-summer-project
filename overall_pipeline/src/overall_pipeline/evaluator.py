...
from .core.data import database_path, read_json, schema_map
from .core.sql import Executor, Judge, reward
...
def _prepare_output(cfg, output, rows, devices):
    ...
    if manifest_path.exists():
        ...
        if manifest.get("signature") != signature:
            raise ValueError("Existing evaluation output was created with different config/data")
    else:
...
def _structural_metrics(cfg, rows, records):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "overall_pipeline.core.spider_metrics"],
...
def evaluate(cfg, output_dir, devices, limit=None):
    ...
    if cfg.evaluation.evaluator_path:
        ...
        subprocess.run([sys.executable, "-m", "overall_pipeline.core.spider_metrics"],
