...
from .core.data import database_path
from .core.protocol import action_messages
from .core.rollout import (Trajectory, answer_only_messages, initial_messages,
                           parse_action)
...
def generate(policy, example, schema, executor, database_dir, rollout_cfg):
    ...
    for _ in range(rollout_cfg.max_intermediate + 1):
        requested = ("answer" if len(result.intermediate) >= rollout_cfg.max_intermediate
                     else None)
