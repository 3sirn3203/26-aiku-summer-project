...
from .core.config import ModelConfig as CoreModelConfig, RolloutConfig, SQLConfig
...
def _find_project_root(start):
    for directory in (start, *start.parents):
        if (directory / "pyproject.toml").is_file():
            return directory
    raise FileNotFoundError(f"Could not find project pyproject.toml above {start}")
...
def _resolve_paths(cfg, project_root):
    cfg.data.test_json = _resolve(project_root, cfg.data.test_json)
    cfg.data.test_tables = _resolve(project_root, cfg.data.test_tables)
    cfg.data.test_database_dir = _resolve(project_root, cfg.data.test_database_dir)
    cfg.evaluation.evaluator_path = _resolve(project_root, cfg.evaluation.evaluator_path)
    cfg.evaluation.nltk_data = _resolve(project_root, cfg.evaluation.nltk_data)
    cfg.evaluation.test_suite_database_dir = _resolve(
        project_root, cfg.evaluation.test_suite_database_dir)
    if cfg.model.adapter_name_or_path:
        cfg.model.adapter_name_or_path = _resolve(
            project_root, cfg.model.adapter_name_or_path)
    if cfg.model.tokenizer_name_or_path and Path(cfg.model.tokenizer_name_or_path).exists():
        cfg.model.tokenizer_name_or_path = _resolve(
            project_root, cfg.model.tokenizer_name_or_path)
    if cfg.model.chat_template:
        cfg.model.chat_template = _resolve(project_root, cfg.model.chat_template)
