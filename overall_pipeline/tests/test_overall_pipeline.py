...
from overall_pipeline.config import (Config, DataConfig, EvaluationConfig, GenerationConfig,
                                     ModelConfig, load_config)
from overall_pipeline.evaluator import _summarize, _validate_suite, evaluate, rescore
from overall_pipeline.generation import generate
from overall_pipeline.model import load_evaluation_policy
from overall_pipeline.core.protocol import action_messages
from overall_pipeline.core.model import Turn
from overall_pipeline.core.rollout import answer_only_messages, initial_messages
from overall_pipeline.core.sql import Executor
from overall_pipeline.core.config import SQLConfig
...
class OverallPipelineTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_checked_in_modes(self):
        ...
        configs = sorted((repository / "configs").glob("*.json"))
        self.assertEqual(len(configs), 8)
        for path in configs:
            cfg = load_config(path)
            expected = ("multi_turn", 3) if "multi_turn" in path.name else ("single_turn", 0)
            self.assertEqual((cfg.generation.mode, cfg.generation.max_intermediate), expected)
    ...
    def test_adapter_is_loaded_on_configured_base(self):
        ...
        with patch("overall_pipeline.model.AutoTokenizer.from_pretrained", return_value=tokenizer), \
             patch("overall_pipeline.model.AutoModelForCausalLM.from_pretrained", return_value=base), \
            ...
            policy = load_evaluation_policy(cfg, "cpu")
    ...
    def test_test_only_generation_scoring_and_reports(self):
        ...
        with patch("overall_pipeline.evaluator.GenerationPool", FakePool):
