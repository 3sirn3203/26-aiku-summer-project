...
@dataclass
class TrainConfig:
    output_dir: str = "rl/outputs/default"
    seed: int = 42
    max_steps: int = 100
    max_groups: int = 2000
...
def load_config(path):
    ...
    if root is None:
        raise FileNotFoundError(f"Could not find project pyproject.toml above {source.parent}")
    def resolve(value):
        if value is None:
            return None
        candidate = Path(value).expanduser()
        return str(candidate if candidate.is_absolute() else (root / candidate).resolve())
    for name in ("train_json", "dev_json", "test_json", "train_others_json",
                 "test_database_dir", "test_tables", "prepared_dir", "database_dir", "tables"):
        value = getattr(config.data, name)
        setattr(config.data, name, resolve(value))
    for name in ("evaluator_path", "nltk_data", "suite_database_dir"):
        setattr(config.sql, name, resolve(getattr(config.sql, name)))
    config.evaluation.dev_suite_database_dir = resolve(config.evaluation.dev_suite_database_dir)
    config.evaluation.test_suite_database_dir = resolve(config.evaluation.test_suite_database_dir)
    config.train.output_dir = resolve(config.train.output_dir)
    if config.model.adapter_name_or_path:
        config.model.adapter_name_or_path = resolve(config.model.adapter_name_or_path)
    if config.model.chat_template:
        config.model.chat_template = resolve(config.model.chat_template)
    config.validate()
    return config
