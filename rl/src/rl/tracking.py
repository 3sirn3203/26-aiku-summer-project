...
class ExperimentTracker:
    ...
    def _start(self, cfg, output, state, resume, job_type, checkpoint):
        ...
        try:
            ...
            raise RuntimeError("Install tracking support with the root project RL dependencies") from exc
        ...
        if resume_run:
            # A crash can happen after the latest periodic checkpoint has been
            # written. Recompute that short tail locally, but do not duplicate
            # it in the resumed W&B run; logging continues at the first new
            # optimizer/group step.
            self.resume_cutoffs = {
                "optimizer_step": int(self.run.summary.get("latest_optimizer_step", 0) or 0),
                "group_step": int(self.run.summary.get("group_step", 0) or 0),
            }
    ...
    def log(self, prefix, metrics, state):
        if self.run:
            if self.resume_cutoffs:
                if (prefix == "rollout" and
                        state.get("groups", 0) <= self.resume_cutoffs["group_step"]):
                    return
                if (prefix != "rollout" and
                        state.get("step", 0) <= self.resume_cutoffs["optimizer_step"]):
                    return
