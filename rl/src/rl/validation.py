...
class Validation:
    ...
    def publish_best(self):
        ...
        target = Path(self.record["best_checkpoint"])
        link = self.output / "best_adapter"
        temporary = self.output / ".best_adapter.tmp"
        if temporary.is_symlink():
            temporary.unlink()
        temporary.symlink_to(target.relative_to(self.output), target_is_directory=True)
