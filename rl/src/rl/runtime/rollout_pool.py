            ...
            if message[0] == "evaluate":
                ...
                with preserve_inference_state(model):
                    trajectory = rollout(
                        ...
                        sample=False, evaluate_suite=bool(eval_cfg.sql.suite_database_dir),
                        policy_version=version)
                results.put(("evaluation", job_id, trajectory))
                continue
            _, job_id, example, schema, seed, expected_version = message
            if version != expected_version:
...
class RolloutPool:
    ...
    def generate(self, example, schema, group_id, policy_version):
        for index in range(self.cfg.rollout.group_size):
            seed = int.from_bytes(hashlib.sha256(
                f"{self.cfg.train.seed}:{group_id}:{index}".encode()).digest()[:8], "big") % (2**31)
            task = ("rollout", index, example, schema, seed, policy_version)
            self.tasks[index % len(self.tasks)].put(task)
        trajectories = {}
        while len(trajectories) < self.cfg.rollout.group_size:
            message = self._get()
            if message[0] != "result":
                raise RuntimeError(f"Unexpected rollout worker message: {message[0]}")
            trajectories[message[1]] = message[2]
        return [trajectories[index] for index in range(self.cfg.rollout.group_size)]
    ...
    def evaluate(self, rows, schemas, policy_version, cfg):
        ...
        with preserve_inference_state(self.policy.model):
            return [rollout(
                ...
                sample=False, evaluate_suite=bool(cfg.sql.suite_database_dir),
