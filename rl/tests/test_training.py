...
from rl.config import Config, ModelConfig, RolloutConfig
from rl.data import prepare
from rl.model import HFPolicy, Turn, action_log_probs, decode_action, load_model, normalize_token_ids
from rl.train import advantages, grpo_terms, rl_optimizer_step, train, update_rl_amp_state
from rl.evaluate import evaluate


def _fsdp_update_smoke(rank, port, result_queue):
    from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
    from rl.runtime.fsdp import _full_state, _optimizer_step
...
def _scripted_rollout_worker(device, cfg, tasks, results):
    from rl.runtime.rollout_pool import _worker
    counter = [0]
...
def _scripted_fsdp_rank(rank, cfg, resume, port, result_queue):
    from rl.runtime.fsdp import _rank_main
    with patch("rl.runtime.rollout_pool._worker", _scripted_rollout_worker):
...
class TrainingTest(unittest.TestCase):
    ...
    def test_group_contains_only_autonomous_rollouts(self):
        free_count = [0]
        ...
        self.assertTrue(all("mode" not in row and "action_trace" not in row
                            for row in records))
    ...
    def test_official_spider_integration(self):
        ...
        from rl.sql import Executor, Judge
        from rl.spider_metrics import run
    ...
    def test_cpu_rollout_worker_version_and_old_log_probs(self):
        from rl.runtime.rollout_pool import RolloutPool
        from rl.data import schema_map
    ...
    def test_cuda_fsdp_launch_and_export(self):
        ...
        self.cfg.validate()
        with patch("rl.runtime.fsdp._rank_main", _scripted_fsdp_rank):
            result = train(self.cfg)
        self.assertEqual(result["step"], 1)
