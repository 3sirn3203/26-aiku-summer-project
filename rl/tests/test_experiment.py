...
from rl.data import load_split, official_manifest, split_config
from rl.tracking import ExperimentTracker
from rl.validation import Validation
...
class ExperimentTest(unittest.TestCase):
    ...
    def test_online_resume_suppresses_recomputed_wandb_tail(self):
        with tempfile.TemporaryDirectory() as tmp:
            cfg = Config()
            cfg.tracking.backend = "wandb"
            cfg.tracking.resume_run = True
            mock = MagicMock()
            run = SimpleNamespace(
                id="parent", entity="team",
                summary={"latest_optimizer_step": 309, "group_step": 751},
                define_metric=MagicMock(), log=MagicMock(), finish=MagicMock(),
                config=MagicMock(),
            )
            mock.init.return_value = run
            state = {"step": 300, "groups": 729,
                     "tracking": {"run_id": "parent", "project": "rl-rrcm-style"}}
            tracker = ExperimentTracker()
            with patch.dict("sys.modules", {"wandb": mock}):
                tracker.start(cfg, tmp, state, resume="checkpoint-300")
                tracker.log("train", {"loss": 1.0}, {"step": 301, "groups": 730})
                tracker.log("rollout", {"reward_mean": 0.5}, {"step": 309, "groups": 751})
                tracker.log("rollout", {"reward_mean": 0.6}, {"step": 309, "groups": 752})
                tracker.log("train", {"loss": 0.5}, {"step": 310, "groups": 752})
            self.assertEqual(mock.init.call_args.kwargs["id"], "parent")
            self.assertEqual(mock.init.call_args.kwargs["resume"], "must")
            self.assertEqual(run.log.call_count, 2)
            self.assertEqual(run.log.call_args_list[0].args[0]["group_step"], 752)
            self.assertEqual(run.log.call_args_list[1].args[0]["optimizer_step"], 310)
